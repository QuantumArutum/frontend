...
describe('兼容性测试', () => {
  ...
  describe('浏览器兼容性测试', () => {
    ...
    it('应该正确处理CSS Grid和Flexbox', () => {
      // 检查Grid布局
      cy.get('[data-testid="auction-grid"]')
  ...
  describe('响应式设计测试', () => {
    ...
    viewports.forEach(({ device, width, height }) => {
      it(`应该在${device}设备上正确显示`, () => {
        ...
        // 检查拍卖卡片布局
        cy.get('[data-testid="auction-card"]').first().then(($card) => {
    ...
    it('应该支持触摸设备交互', () => {
      ...
      // 检查触摸友好的按钮大小
      cy.get('[data-testid="bid-button"]').first()
  ...
  describe('国际化兼容性测试', () => {
    ...
    it('应该支持不同时区', () => {
      ...
      // 检查时间显示
      cy.get('[data-testid="auction-end-time"]').first()
        .should('be.visible')
        .and('not.be.empty');
