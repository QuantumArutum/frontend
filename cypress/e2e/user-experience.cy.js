...
describe('用户体验测试', () => {
  ...
  describe('可用性测试', () => {
    it('用户应该能够快速找到并浏览拍卖', () => {
      ...
      // 拍卖卡片应该包含关键信息
      cy.get('[data-testid="auction-card"]').first().within(() => {
    ...
    it('搜索功能应该提供即时反馈', () => {
      ...
      // 应该显示搜索建议或即时结果
      cy.get('[data-testid="search-suggestions"]', { timeout: 1000 })
        .should('be.visible');
  ...
  describe('无障碍测试', () => {
    ...
    it('应该有适当的ARIA标签和角色', () => {
      ...
      // 检查状态信息
      cy.get('[data-testid="auction-status"]').first()
    ...
    it('应该有足够的颜色对比度', () => {
      // 检查主要文本对比度
      cy.get('[data-testid="auction-title"]').first().then(($el) => {
      ...
      // 检查按钮对比度
      cy.get('[data-testid="bid-button"]').first().then(($el) => {
        const color = $el.css('color');
        const backgroundColor = $el.css('background-color');
    ...
    it('应该支持高对比度模式', () => {
      ...
      // 检查元素在高对比度模式下的可见性
      cy.get('[data-testid="auction-card"]').first()
        .should('be.visible')
        .and('have.css', 'border')
        .and('not.equal', 'none');
    });
  ...
  describe('响应式设计测试', () => {
    ...
    devices.forEach(({ name, width, height }) => {
      it(`应该在${name}上正确显示`, () => {
        ...
        // 检查文字大小
        cy.get('[data-testid="auction-title"]').first()
  ...
  describe('性能用户体验测试', () => {
    it('页面加载应该快速', () => {
      ...
      cy.get('[data-testid="auction-list"]').should('be.visible').then(() => {
    ...
    it('交互应该响应迅速', () => {
      ...
      // 搜索建议应该快速出现
      cy.get('[data-testid="search-suggestions"]', { timeout: 500 })
        .should('be.visible');
      
      // 按钮点击应该有即时反馈
      cy.get('[data-testid="filter-button"]').first().click();
      cy.get('[data-testid="filter-button"]').first()
  ...
  describe('内容可读性测试', () => {
    it('文本应该易于阅读', () => {
      // 检查行高
      cy.get('[data-testid="auction-description"]').first()
    ...
    it('重要信息应该突出显示', () => {
      // 价格应该突出
      cy.get('[data-testid="current-price"]').first()
      ...
      // 剩余时间应该醒目
      cy.get('[data-testid="time-remaining"]').first()
    ...
    it('表单应该有清晰的标签和说明', () => {
      ...
      // 应该有帮助文本
      cy.get('[data-testid="bid-help-text"]')
        .should('be.visible')
        .and('not.be.empty');
  ...
  describe('用户反馈和确认', () => {
    ...
    it('应该提供撤销选项', () => {
      ...
      // 应该显示撤销选项
      cy.get('[data-testid="undo-notification"]')
        .should('be.visible')
        .and('contain', '撤销');
