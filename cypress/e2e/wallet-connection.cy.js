...
describe('钱包连接测试', () => {
  ...
  describe('钱包检测和连接', () => {
    ...
    it('应该成功连接MetaMask钱包', () => {
      ...
      cy.get('[data-testid="wallet-address"]')
        .should('be.visible')
        .and('contain', '0x1234...7890');
      cy.get('[data-testid="wallet-balance"]')
        .should('be.visible')
        .and('contain', '2.0000 ETH');
    });
  ...
  describe('网络检测和切换', () => {
    ...
    it('应该检测不支持的网络', () => {
      ...
      // 触发网络变化事件
      cy.window().then((win) => {
        const changeHandler = cy.get('@ethereumOn').args.find(
          args => args[0] === 'chainChanged'
        )[1];
        changeHandler('0x38');
      });
  ...
  describe('出价支付流程测试', () => {
    ...
    it('应该成功提交出价交易', () => {
      ...
      // 应该显示交易哈希
      cy.get('[data-testid="transaction-hash"]')
        .should('be.visible')
        .and('contain', '0xabcdef');
    });
    ...
    it('应该处理交易失败', () => {
      ...
      // 应该提供重试选项
      cy.get('[data-testid="retry-transaction"]')
        .should('be.visible')
        .and('contain', '重试');
    });
  });
  ...
  describe('一口价支付流程测试', () => {
    ...
    it('应该确认一口价购买', () => {
      ...
      // 应该显示确认对话框
      cy.get('[data-testid="buy-now-confirm"]')
        .should('be.visible')
        .and('contain', '确认购买');
      ...
      cy.get('[data-testid="platform-fee"]')
        .should('be.visible')
        .and('contain', '平台手续费');
    });
    ...
    it('应该成功执行一口价购买', () => {
      ...
      // 应该更新拍卖状态
      cy.get('[data-testid="auction-status"]')
        .should('contain', '已售出');
    });
  });
  ...
  describe('钱包状态管理', () => {
    ...
    it('应该处理钱包断开连接', () => {
      ...
      cy.get('[data-testid="wallet-connected"]')
        .should('not.exist');
    });
    ...
    it('应该自动重连已授权的钱包', () => {
      ...
      // 应该自动显示连接状态
      cy.get('[data-testid="wallet-connected"]', { timeout: 5000 })
        .should('be.visible');
    });
  });
