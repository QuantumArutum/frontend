...
describe('用户交互流程测试', () => {
  ...
  describe('完整的用户注册和登录流程', () => {
    ...
    it('应该处理登录错误', () => {
      ...
      // 验证错误消息
      cy.get('[data-testid="login-error"]').should('be.visible').and('contain', '邮箱或密码错误');
    });
  });
  ...
  describe('出价流程测试', () => {
    ...
    it('应该处理出价验证错误', () => {
      ...
      cy.get('[data-testid="bid-error"]').should('be.visible').and('contain', '出价金额过低');

      // 2. 测试无效金额
      cy.get('[data-testid="bid-amount-input"]').clear().type('abc');
      cy.get('[data-testid="confirm-bid-button"]').click();

      cy.get('[data-testid="bid-error"]').should('be.visible').and('contain', '请输入有效金额');
    });
  });
  ...
  describe('KYC验证流程', () => {
    ...
    it('用户应该能够完成KYC验证', () => {
      ...
      // 6. 验证提交成功
      cy.get('[data-testid="kyc-submitted"]').should('be.visible').and('contain', 'KYC申请已提交');
  ...
  describe('用户设置和个人资料管理', () => {
    ...
    it('用户应该能够更新个人资料', () => {
      ...
      // 5. 验证保存成功
      cy.get('[data-testid="settings-saved"]').should('be.visible').and('contain', '设置已保存');
    });
  ...
  describe('错误处理和边界情况', () => {
    it('应该处理网络错误', () => {
      ...
      // 验证错误处理
      cy.get('[data-testid="network-error"]').should('be.visible').and('contain', '网络连接失败');
    ...
    it('应该处理会话过期', () => {
      ...
      // 应该重定向到登录页
      cy.url().should('include', '/login');
      cy.get('[data-testid="session-expired"]').should('be.visible').and('contain', '会话已过期');
    });
  });
});
