...
export const postsService = {
  ...
  addComment: async (
    postId: string,
    comment: { userId: string; userName: string; content: string; parentId?: string }
  ...
  getProposalById: async (proposalId: string): Promise<Proposal | null> => {
    try {
      const response = await apiRequest<Proposal>(
        `${COMMUNITY_API}/governance/proposals/${proposalId}`
      );
      return response.data || null;
    } catch (error) {
...
export const aiEvolutionService = {
  ...
  getAgentById: async (agentId: string): Promise<AIAgent | null> => {
    try {
      const response = await apiRequest<AIAgent>(
        `${COMMUNITY_API}/ai-evolution?type=agent&agentId=${agentId}`
      );
      return response.data || null;
    } catch (error) {
  ...
  getLeaderboard: async (): Promise<
    { rank: number; agent: AIAgent; score: number; trend: 'up' | 'down' | 'stable' }[]
  > => {
    try {
      const response = await apiRequest<
        { rank: number; agent: AIAgent; score: number; trend: 'up' | 'down' | 'stable' }[]
...
export const searchService = {
  ...
  ): Promise<{ posts: Post[]; users: UserProfile[] }> => {
    try {
  ...
  return [
    {
      ...
      lastMessage: {
        ...
        senderId: 'user_2',
        senderName: 'QuantumDev',
        receiverId: userId,
        content: '你好！关于量子安全的问题，我想和你讨论一下。',
        isRead: false,
...
function getDefaultProposals(): Proposal[] {
  return [
    {
      id: '1',
      title: '升级量子安全算法至CRYSTALS-Dilithium v3.1',
      description:
        '提议将当前使用的CRYSTALS-Dilithium算法升级到最新的v3.1版本，以提高签名效率和安全性。',
      status: 'active',
      proposer: 'QuantumCore团队',
      proposerAvatar: 'QC',
      category: '技术升级',
      votes: { for: 969685, against: 264882, abstain: 0 },
      totalVotes: 1234567,
      quorum: 2000000,
      endDate: '2024-12-27',
      tags: ['技术升级', '量子安全'],
    ...
    },
    {
      ...
      title: '设立开发者生态基金',
      description:
        '提议从财库中拨出500万QAU设立开发者生态基金，用于资助优秀的DApp开发项目和技术创新。',
      status: 'pending',
      proposer: 'EcoFund DAO',
      proposerAvatar: 'EF',
      category: '生态发展',
...
function getDefaultDelegates(): Delegate[] {
  return [
    {
      id: '1',
      name: 'QuantumFoundation',
      avatar: 'QF',
      description: '官方基金会，专注于量子区块链技术发展和生态建设',
      delegatedVotes: 1234567,
    ...
    },
    {
      ...
      name: 'CryptoDev联盟',
      avatar: 'CD',
      description: '开发者组织，致力于推动DApp生态发展和技术创新',
      delegatedVotes: 987654,
    ...
    },
    {
      ...
      avatar: 'QS',
      description: '安全专家团队，专注于量子安全和密码学研究',
      delegatedVotes: 756432,
