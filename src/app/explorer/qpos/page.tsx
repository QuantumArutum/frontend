...
export default function QPOSPage() {
  ...
  const slotProgress = qposStatus
    ? ((qposStatus.slotInEpoch + 1) / qposStatus.slotsPerEpoch) * 100
    : 0;
      ...
      {qposStatus && (
        ...
        >
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-gray-400 text-sm">
                {t('explorer.qpos.epoch_progress', { epoch: qposStatus.currentEpoch })}
              </span>
              <span className="text-purple-400 font-mono text-sm">
                {t('explorer.qpos.slot_info', {
                  current: qposStatus.slotInEpoch + 1,
                  total: qposStatus.slotsPerEpoch,
                })}
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard
              label={t('explorer.qpos.stats.current_slot')}
              value={qposStatus.currentSlot}
              icon={Clock}
              color="text-cyan-400"
            />
            <StatCard
              label={t('explorer.qpos.stats.current_epoch')}
              value={qposStatus.currentEpoch}
              icon={Layers}
              color="text-purple-400"
            />
            <StatCard
              label={t('explorer.qpos.stats.justified_epoch')}
              value={qposStatus.justifiedEpoch}
              icon={CheckCircle}
              color="text-blue-400"
            />
            <StatCard
              label={t('explorer.qpos.stats.finalized_epoch')}
              value={qposStatus.finalizedEpoch}
              icon={Shield}
      ...
      {activeTab === 'overview' && qposStatus && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  ...
                  <p className="text-white font-mono text-lg">
                    {qposStatus.epochSummary.totalAttestations}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  <p className="text-gray-400 text-xs">{t('explorer.qpos.participation_rate')}</p>
                  <p className="text-emerald-400 font-mono text-lg">
                    {(qposStatus.epochSummary.participationRate * 100).toFixed(1)}%
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  <p className="text-gray-400 text-xs">{t('explorer.qpos.justified')}</p>
                  <p
                    className={`font-mono text-lg ${qposStatus.epochSummary.isJustified ? 'text-green-400' : 'text-gray-400'}`}
                  >
                    {qposStatus.epochSummary.isJustified
                      ? t('explorer.qpos.yes')
                      : t('explorer.qpos.no')}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  ...
                  <p
                    className={`font-mono text-lg ${qposStatus.epochSummary.isFinalized ? 'text-green-400' : 'text-gray-400'}`}
                  >
                    {qposStatus.epochSummary.isFinalized
                      ? t('explorer.qpos.yes')
                      : t('explorer.qpos.no')}
                  </p>
                </div>
          ...
          <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                ...
                <p className="text-purple-400 font-mono text-sm truncate">
                  {qposStatus.currentProposer
                    ? `0x${qposStatus.currentProposer.slice(0, 8)}...`
                    : '-'}
                </p>
              </div>
              <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                ...
                <p
                  className={`font-mono text-lg ${qposStatus.slashedValidators.length > 0 ? 'text-red-400' : 'text-gray-400'}`}
                >
            ...
            <button
              onClick={() => router.push('/explorer/validators')}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  ...
                  <p className="text-green-400 font-mono">
                    {explorerUtils.formatWei(qposStatus.lastEpochRewards.totalRewards)}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  <p className="text-gray-400 text-xs">{t('explorer.qpos.total_penalties')}</p>
                  <p className="text-red-400 font-mono">
                    {explorerUtils.formatWei(qposStatus.lastEpochRewards.totalPenalties)}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  <p className="text-gray-400 text-xs">{t('explorer.qpos.participating_stake')}</p>
                  <p className="text-cyan-400 font-mono">
                    {explorerUtils.formatWei(qposStatus.lastEpochRewards.participatingStake)}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  <p className="text-gray-400 text-xs">{t('explorer.qpos.total_stake')}</p>
                  <p className="text-white font-mono">
                    {explorerUtils.formatWei(qposStatus.lastEpochRewards.totalStake)}
                  </p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-white/5 border-b border-white/10">
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">
                      {t('explorer.qpos.columns.epoch')}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">
                      {t('explorer.qpos.columns.slots')}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">
                      {t('explorer.qpos.columns.status')}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">
                      {t('explorer.qpos.columns.justified')}
                    </th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">
                      {t('explorer.qpos.columns.finalized')}
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {epochsData?.epochs.map((epoch) => (
                    <tr
                      key={epoch.epoch}
                      className={`hover:bg-white/5 transition-colors ${epoch.isCurrent ? 'bg-purple-500/5' : ''}`}
                    >
                      <td className="px-6 py-4">
                        <span
                          className={`font-mono font-medium ${epoch.isCurrent ? 'text-purple-400' : 'text-cyan-400'}`}
                        >
                          {epoch.epoch}
                        </span>
                        {epoch.isCurrent && (
                          <span className="ml-2 text-xs bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded">
                            {t('explorer.qpos.current')}
                          </span>
                        )}
                      </td>
                  ...
                  {!epochsData && (
                    <tr className="animate-pulse">
                      <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                        {t('explorer.qpos.loading_epochs')}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6">
              ...
              <div className="space-y-4">
                <DetailRow
                  label={t('explorer.qpos.head_root')}
                  value={qposStatus.advanced.forkChoice.head}
                  mono
                />
                <DetailRow
                  label={t('explorer.qpos.justified_root')}
                  value={qposStatus.advanced.forkChoice.justifiedRoot}
                  mono
                />
                <DetailRow
                  label={t('explorer.qpos.finalized_root')}
                  value={qposStatus.advanced.forkChoice.finalizedRoot}
                  mono
                />
              </div>
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-white/5">
                  ...
                  <span
                    className={`px-2 py-1 rounded text-xs ${qposStatus.advanced.inactivityLeakActive ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}
                  >
                    {qposStatus.advanced.inactivityLeakActive
                      ? t('explorer.qpos.active')
                      : t('explorer.qpos.inactive')}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-white/5">
                  <span className="text-gray-400">{t('explorer.qpos.pending_withdrawals')}</span>
                  <span className="text-white font-mono">
                    {qposStatus.advanced.pendingWithdrawals}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-gray-400">{t('explorer.qpos.sync_committee_size')}</span>
                  <span className="text-white font-mono">
                    {qposStatus.syncCommittee?.size || 512}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                  ...
                  <p className="text-yellow-400 font-mono text-sm truncate">
                    {qposStatus.proposerBoost.blockRoot}
                  </p>
                </div>
                <div className="bg-white/5 rounded-lg p-3 border border-white/5">
