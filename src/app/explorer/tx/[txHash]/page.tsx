    ...
    <div className="max-w-7xl mx-auto px-5">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 mb-8">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${
          tx?.status === 'success' ? 'bg-gradient-to-r from-emerald-500 to-teal-500 shadow-emerald-500/20' :
          tx?.status === 'failed' ? 'bg-gradient-to-r from-red-500 to-pink-500 shadow-red-500/20' :
          'bg-gradient-to-r from-yellow-500 to-orange-500 shadow-yellow-500/20'
        }`}>
          {tx?.status === 'success' ? <CheckCircle className="w-7 h-7 text-white" /> :
           tx?.status === 'failed' ? <XCircle className="w-7 h-7 text-white" /> :
           <Clock className="w-7 h-7 text-white" />}
        </div>
        <div>
          <h1 className="text-3xl font-bold text-white">Transaction Details</h1>
          <p className="text-gray-400 font-mono text-sm">{explorerUtils.truncateHash(txHash, 16, 12)}</p>
        </div>
        ...
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-4">
              <DetailRow label="Transaction Hash" value={tx.hash} mono copyable onCopy={() => copyToClipboard(tx.hash)} />
              <DetailRow label="Block" value={`#${explorerUtils.formatBlockNumber(tx.blockNumber).toLocaleString()}`} 
                link onClick={() => router.push(`/explorer/block/${explorerUtils.formatBlockNumber(tx.blockNumber)}`)} />
              {tx.timestamp && <DetailRow label="Time" value={explorerUtils.formatTimestamp(tx.timestamp)} />}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/5">
              <div className="flex-1 text-center md:text-left">
                ...
                <Link href={`/explorer/address/${tx.from}`} className="text-indigo-400 hover:text-indigo-300 font-mono text-sm break-all block">
                  {tx.from}
                </Link>
              </div>
              ...
              <div className="flex-1 text-center md:text-right">
                ...
                {tx.to ? (
                    <Link href={`/explorer/address/${tx.to}`} className="text-indigo-400 hover:text-indigo-300 font-mono text-sm break-all block">
                        {tx.to}
                    </Link>
                ) : (
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                    ...
                    <p className="text-xl font-bold text-white">{explorerUtils.formatWei(tx.value, 6)} QAU</p>
                </div>
                <div className="p-4 bg-white/5 rounded-xl border border-white/5">
...
function DetailRow({ label, value, mono, copyable, onCopy, link, onClick }: { label: string; value: string; mono?: boolean; copyable?: boolean; onCopy?: () => void; link?: boolean; onClick?: () => void }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center py-3 border-b border-white/5 last:border-0">
      ...
      <div className="flex items-center gap-2 flex-1">
        {link ? (
            <button onClick={onClick} className={`text-sm break-all hover:underline ${mono ? 'font-mono text-cyan-400' : 'text-cyan-400'}`}>
                {value}
            </button>
        ) : (
