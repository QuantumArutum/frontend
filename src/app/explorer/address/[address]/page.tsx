...
import { User, ArrowLeft, Copy, Wallet, Activity, Code, ArrowRight, ArrowUpRight } from 'lucide-react';
import Link from 'next/link';
import { explorerService, Address, Transaction, explorerUtils } from '../../../../services/explorerService';
    ...
    <div className="max-w-7xl mx-auto px-5">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 mb-8">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${
          addressInfo?.isContract ? 'bg-gradient-to-r from-purple-500 to-pink-500 shadow-purple-500/20' : 'bg-gradient-to-r from-blue-500 to-cyan-500 shadow-blue-500/20'
        }`}>
          {addressInfo?.isContract ? <Code className="w-7 h-7 text-white" /> : <User className="w-7 h-7 text-white" />}
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-white">{addressInfo?.isContract ? 'Contract Address' : 'Wallet Address'}</h1>
            {addressInfo?.isContract && (
              <span className="px-2 py-1 bg-purple-500/20 text-purple-400 text-xs rounded border border-purple-500/30">Smart Contract</span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-1">
            <p className="text-gray-400 font-mono text-sm break-all">{address}</p>
            <button onClick={() => copyToClipboard(address)} className="p-1 rounded hover:bg-white/10 text-gray-500 hover:text-white transition-colors">
        ...
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="bg-gradient-to-br from-blue-500/10 to-cyan-500/10 backdrop-blur-md rounded-2xl border border-cyan-500/30 p-6">
              ...
              <p className="text-3xl font-bold text-white">{explorerUtils.formatWei(addressInfo.balance, 4)} QAU</p>
              <p className="text-gray-400 text-sm mt-1">≈ ${addressInfo.balanceUSD?.toLocaleString() || '0'} USD</p>
            </div>
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6">
              ...
              <p className="text-3xl font-bold text-white">{addressInfo.transactionCount.toLocaleString()}</p>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-white/10">
                                ...
                                <th className="px-6 py-3 text-left text-sm font-medium text-gray-400">Method</th>
                                <th className="px-6 py-3 text-left text-sm font-medium text-gray-400">Block</th>
                            ...
                            {transactions.map((tx) => (
                                <tr key={tx.hash} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-3">
                                        <Link href={`/explorer/tx/${tx.hash}`} className="text-cyan-400 hover:text-cyan-300 font-mono text-sm">
                                            {explorerUtils.truncateHash(tx.hash, 8, 6)}
                                        </Link>
                                    </td>
                                    <td className="px-6 py-3">
                                        <span className="px-2 py-1 rounded bg-white/10 text-xs text-gray-300 border border-white/10">Transfer</span>
                                    </td>
                                    <td className="px-6 py-3">
                                        <Link href={`/explorer/block/${explorerUtils.formatBlockNumber(tx.blockNumber)}`} className="text-indigo-400 hover:text-indigo-300 text-sm">
                                    ...
                                    <td className="px-6 py-3">
                                        <div className="flex items-center gap-2">
                                            {tx.from.toLowerCase() === address.toLowerCase() ? (
                                                <span className="p-1 rounded bg-orange-500/20 text-orange-400 text-xs font-bold">OUT</span>
                                            ) : (
                                                <span className="p-1 rounded bg-green-500/20 text-green-400 text-xs font-bold">IN</span>
                                            )}
                                            {tx.to ? (
                                    ...
                                    <td className="px-6 py-3 text-white text-sm">{explorerUtils.formatWei(tx.value, 4)} QAU</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
