  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      if (!formData.verificationCode.trim())
        newErrors.verificationCode = t('auth.register.validation.code_required');
      else if (formData.verificationCode.length !== 6)
        newErrors.verificationCode = t('auth.register.validation.code_invalid');
    }
    if (step === 2) {
      if (!formData.password) newErrors.password = t('auth.register.validation.password_required');
      else if (formData.password.length < 8)
        newErrors.password = t('auth.register.validation.password_min_length');
      else if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(formData.password))
        newErrors.password = t('auth.register.validation.password_complexity');
      if (!formData.confirmPassword)
        newErrors.confirmPassword = t('auth.register.validation.confirm_password_required');
      else if (formData.password !== formData.confirmPassword)
        newErrors.confirmPassword = t('auth.register.validation.password_mismatch');
    }
    if (step === 3) {
      if (!formData.agreeTerms) newErrors.agreeTerms = t('auth.register.validation.terms_required');
      if (!formData.agreePrivacy)
        newErrors.agreePrivacy = t('auth.register.validation.privacy_required');
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrors({ submit: data.message || t('auth.register.register_failed') });
      }
    } catch {
      setErrors({ submit: t('auth.register.network_error') });
    } finally {
      setLoading(false);
    }
  };
        ...
        return (
          ...
          >
            <h3 className="text-lg font-semibold mb-4" style={{ color: colors.text.primary }}>
              {t('auth.register.step_email_verify')}
            </h3>
            <div>
              <label
            ...
            <div>
              ...
              <div className="flex gap-3">
                <input
                  ...
                  onBlur={() => setFocusedField(null)}
                  maxLength={6}
                  className="flex-1 px-4 py-3 rounded-xl outline-none text-center tracking-widest text-lg"
                  style={inputStyle('verificationCode', !!errors.verificationCode)}
                  placeholder="000000"
                />
                <button
                  type="button"
                  ...
                  style={{
                    background:
                      countdown > 0
                        ? colors.glass.medium
                        : `linear-gradient(135deg, ${colors.secondary} 0%, ${colors.accent.cyan} 100%)`,
                    color: colors.text.primary,
              ...
              {codeSent && (
                <p className="text-sm mt-2" style={{ color: colors.status.success }}>
                  {t('auth.register.code_sent_hint')}
                </p>
              )}
            </div>
          </motion.div>
            ...
            <div>
              ...
              <div className="relative">
                <Lock
                  className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5"
                  style={{
                    color: focusedField === 'password' ? colors.secondary : colors.text.muted,
                  }}
                />
                <input
            ...
            <div>
              ...
              <div className="relative">
                <Lock
                  ...
                  style={{
                    color:
                      focusedField === 'confirmPassword' ? colors.secondary : colors.text.muted,
                  }}
                />
                <input
          ...
          >
            ...
            <div className="space-y-4">
              <label className="flex items-start cursor-pointer">
                ...
                <span className="ml-3 text-sm" style={{ color: colors.text.secondary }}>
                  ...
                  <Link href="/terms" style={{ color: colors.secondary }}>
              ...
              <label className="flex items-start cursor-pointer">
                ...
                />
                <span className="ml-3 text-sm" style={{ color: colors.text.secondary }}>
                  {t('auth.register.agree_privacy')}{' '}
                  <Link href="/privacy" style={{ color: colors.secondary }}>
              ...
              <label className="flex items-start cursor-pointer">
                ...
                />
                <span className="ml-3 text-sm" style={{ color: colors.text.secondary }}>
                  {t('auth.register.agree_marketing')}
                </span>
              </label>
            </div>
            <div
            ...
            >
              ...
              <span className="text-sm" style={{ color: colors.status.success }}>
                {t('auth.register.security_guarantee')}
              </span>
            </div>
          </motion.div>
        );
        ...
        >
            ...
            {[1, 2, 3].map((step) => (
              <div key={step} className="flex items-center">
                <div
                  ...
                  style={{
                    background: step <= currentStep ? colors.secondary : colors.background.tertiary,
                    color: colors.text.primary,
                  }}
                >
                ...
                {step < 3 && (
                  <div
                    ...
                    style={{
                      background:
                        step < currentStep ? colors.secondary : colors.background.tertiary,
                    }}
                  />
                )}
              </div>
          ...
          {errors.submit && (
            ...
            >
              ...
              <span style={{ color: colors.status.error, fontSize: typography.fontSize.sm }}>
                {errors.submit}
              </span>
            </motion.div>
          )}
          {success && (
            ...
            >
              ...
              <span style={{ color: colors.status.success, fontSize: typography.fontSize.sm }}>
                {success}
              </span>
            </motion.div>
          )}
                  ...
                  >
                    {loading ? (
                      <div
                        ...
                        style={{ borderColor: `${colors.text.primary} transparent` }}
                      />
                    ) : (
                      <>
