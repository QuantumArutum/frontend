...
export const POST = createSecureHandler(
  async (request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      const result = await callRPC('qau_stakeFarm', [farmId, userAddress, amountWei]);

      if (!result.success) {
        return errorResponse(result.error || 'Farm stake failed', 500);
      }
      ...
      return successResponse({
        ...
        message: `Successfully staked LP tokens in ${farmId}`,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      console.error('Farm stake error:', error);
      const message = error instanceof Error ? error.message : 'Failed to stake in farm';
