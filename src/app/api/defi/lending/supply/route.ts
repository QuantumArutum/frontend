...
export const POST = createSecureHandler(
  async (request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      const result = await callRPC('qau_supply', [poolId, userAddress, amountWei]);

      if (!result.success) {
        return errorResponse(result.error || 'Supply failed', 500);
      }
      ...
      return successResponse({
        ...
        message: `Successfully supplied to ${poolId}`,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      console.error('Supply error:', error);
      const message = error instanceof Error ? error.message : 'Failed to supply';
