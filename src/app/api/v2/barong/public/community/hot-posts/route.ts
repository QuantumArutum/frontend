/**
 * Hot Posts API
 * Get posts sorted by hot score (Reddit-style algorithm) - 优化版 + 缓存
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 生成缓存键
    const cacheKey = generateCacheKey('hot-posts', { page, limit, timeRange });

    // 使用缓存包装数据获取逻辑
    const result = await withCache(
      cacheKey,
      CacheTTL.TWO_MINUTES, // 热门帖子缓存2分钟
      async () => {
