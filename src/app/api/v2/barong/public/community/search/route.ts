...
export async function GET(request: NextRequest) {
  try {
    ...
    // 生成缓存键
    const cacheKey = generateCacheKey('search', { query, type, limit, offset });

    // 使用缓存包装数据获取逻辑
    const results = await withCache(
      cacheKey,
      CacheTTL.FIVE_MINUTES, // 搜索结果缓存5分钟
      async () => {
        ...
        try {
          ...
          data.total = data.posts.length + data.users.length + data.tags.length;
