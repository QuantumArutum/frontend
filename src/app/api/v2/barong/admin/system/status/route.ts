...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sql) {
      try {
        ...
        if (testResult) {
          ...
          try {
            const [sessionsCount] = await sql`SELECT COUNT(*) as count FROM admin_sessions WHERE expires_at > NOW()`;
    ...
    return NextResponse.json({
      ...
      data: {
        database: { status: 'disconnected', error: error.message, tables: { users: 0, posts: 0, sessions: 0 } },
        server: {
        ...
        environment: { node_env: process.env.NODE_ENV || 'development', database_configured: false, vercel: false },
        recent_errors: [],
