    ...
    if (sql) {
      ...
      if (type === 'events') {
        ...
        if (status && status !== 'all') {
          ...
          `;
          countResult =
            await sql`SELECT COUNT(*) as total FROM security_events WHERE status = ${status}`;
          ...
          `;
          countResult =
            await sql`SELECT COUNT(*) as total FROM security_events WHERE severity = ${severity}`;
        } else {
          events = await sql`
            SELECT * FROM security_events 
            ORDER BY created_at DESC 
      ...
      if (type === 'stats') {
        ...
        // Get event counts
        const [openEvents] =
          await sql`SELECT COUNT(*) as count FROM security_events WHERE status = 'open'`;
        const [criticalEvents] =
          await sql`SELECT COUNT(*) as count FROM security_events WHERE severity = 'critical' AND status != 'resolved'`;
        const [totalBlocked] =
          await sql`SELECT COUNT(*) as count FROM ip_rules WHERE rule_type = 'blacklist'`;
