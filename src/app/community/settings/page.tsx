...
export default function CommunitySettingsPage() {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data) {
            ...
            setSettings({
              ...
              emailNotifications:
                apiSettings.email_notifications || defaultSettings.emailNotifications,
              pushNotifications:
                apiSettings.push_notifications || defaultSettings.pushNotifications,
  ...
  const handleChangePassword = async () => {
    ...
    if (passwordData.new.length < 8) {
      alert(
        t('settings_page.security.password_too_short', 'Password must be at least 8 characters')
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        alert(
          data.message ||
            t('settings_page.security.password_change_failed', 'Failed to change password')
        );
      }
    } catch (err) {
  ...
  const handleDeleteAccount = async () => {
    const password = prompt(
      t('settings_page.delete_modal.enter_password', 'Enter your password to confirm:')
    );
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        alert(
          data.message || t('settings_page.delete_modal.delete_failed', 'Failed to delete account')
        );
      }
    } catch (err) {
  ...
  const tabs: { id: SettingsTab; label: string; icon: React.ElementType }[] = [
    ...
    {
      id: 'notifications',
      label: t('settings_page.tabs.notifications', 'Notifications'),
      icon: Bell,
    },
      ...
      <div className="pt-12 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            {t('settings_page.profile.display_name', 'Display Name')}
          </label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            {t('settings_page.profile.bio', 'Bio')}
          </label>
          <textarea
            value={settings.bio}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            {t('settings_page.profile.location', 'Location')}
          </label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            {t('settings_page.profile.website', 'Website')}
          </label>
          <div className="relative">
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {t('settings_page.security.email', 'Email Address')}
              </p>
              <p className="text-sm text-gray-400">{userInfo?.email}</p>
            </div>
          </div>
          <span className="px-3 py-1 bg-green-500/20 text-green-400 text-sm rounded-full">
            {t('settings_page.security.verified', 'Verified')}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {t('settings_page.security.password', 'Password')}
              </p>
              <p className="text-sm text-gray-400">
                {t('settings_page.security.last_changed', 'Last changed 30 days ago')}
              </p>
            </div>
          </div>
          <button
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {t('settings_page.security.two_factor', 'Two-Factor Authentication')}
              </p>
              <p className="text-sm text-gray-400">
                {t('settings_page.security.two_factor_desc', 'Add an extra layer of security')}
              </p>
            </div>
          </div>
          <button
          ...
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.twoFactorEnabled ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
      </div>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {t('settings_page.security.login_alerts', 'Login Alerts')}
              </p>
              <p className="text-sm text-gray-400">
                {t('settings_page.security.login_alerts_desc', 'Get notified of new logins')}
              </p>
            </div>
          </div>
          <button
          ...
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.loginAlerts ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
      </div>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <h3 className="text-white font-medium mb-4">
          {t('settings_page.security.connected_accounts', 'Connected Accounts')}
        </h3>
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
            ...
            <span className="text-green-400 text-sm">
              {t('settings_page.security.connected', 'Connected')}
            </span>
          </div>
        </div>
      </div>
          ...
          {[
            {
              key: 'replies',
              icon: MessageSquare,
              label: t('settings_page.notifications.replies', 'Replies to your posts'),
            },
            {
              key: 'likes',
              icon: Heart,
              label: t('settings_page.notifications.likes', 'Likes on your posts'),
            },
            {
              key: 'follows',
              icon: UserPlus,
              label: t('settings_page.notifications.follows', 'New followers'),
            },
            {
              key: 'mentions',
              icon: AtSign,
              label: t('settings_page.notifications.mentions', 'Mentions'),
            },
            {
              key: 'newsletter',
              icon: Mail,
              label: t('settings_page.notifications.newsletter', 'Newsletter'),
            },
            {
              key: 'announcements',
              icon: Bell,
              label: t('settings_page.notifications.announcements', 'Platform announcements'),
            },
          ].map((item) => (
            <div
              key={item.key}
                ...
                onClick={() =>
                  setSettings((prev) => ({
                    ...
                    emailNotifications: {
                      ...
                      [item.key]:
                        !prev.emailNotifications[item.key as keyof typeof prev.emailNotifications],
                    },
                  }))
                }
                className={`w-10 h-5 rounded-full transition-colors relative ${
                  settings.emailNotifications[item.key as keyof typeof settings.emailNotifications]
                    ? 'bg-green-500'
                    : 'bg-gray-600'
                }`}
              >
        ...
        <div className="space-y-3">
          {[
            ...
            },
            {
              key: 'directMessages',
              icon: MessageSquare,
              label: t('settings_page.notifications.direct_messages', 'Direct messages'),
            },
          ].map((item) => (
            <div
              key={item.key}
                ...
                onClick={() =>
                  setSettings((prev) => ({
                    ...
                    pushNotifications: {
                      ...
                      [item.key]:
                        !prev.pushNotifications[item.key as keyof typeof prev.pushNotifications],
                    },
                  }))
                }
                className={`w-10 h-5 rounded-full transition-colors relative ${
                  settings.pushNotifications[item.key as keyof typeof settings.pushNotifications]
                    ? 'bg-green-500'
                    : 'bg-gray-600'
                }`}
              >
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {t('settings_page.notifications.quiet_hours', 'Quiet Hours')}
              </p>
              <p className="text-sm text-gray-400">
                {t(
                  'settings_page.notifications.quiet_hours_desc',
          ...
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.quietHours.enabled ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-white/10">
            <div className="flex-1">
              <label className="block text-sm text-gray-400 mb-1">
                {t('settings_page.notifications.start_time', 'Start')}
              </label>
              <input
                type="time"
            ...
            <div className="flex-1">
              <label className="block text-sm text-gray-400 mb-1">
                {t('settings_page.notifications.end_time', 'End')}
              </label>
              <input
                type="time"
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        ...
        <div className="space-y-2">
          {[
            {
              value: 'public',
              label: t('settings_page.privacy.public', 'Public'),
              desc: t('settings_page.privacy.public_desc', 'Anyone can view your profile'),
            },
            {
              value: 'members',
              label: t('settings_page.privacy.members_only', 'Members Only'),
              desc: t('settings_page.privacy.members_desc', 'Only logged-in members can view'),
            },
            {
              value: 'private',
              label: t('settings_page.privacy.private', 'Private'),
              desc: t('settings_page.privacy.private_desc', 'Only you can view your profile'),
            },
          ].map((option) => (
            <label
              key={option.value}
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {t('settings_page.privacy.show_online', 'Show Online Status')}
              </p>
              <p className="text-sm text-gray-400">
                {t('settings_page.privacy.show_online_desc', 'Let others see when you are online')}
              </p>
            </div>
          </div>
          <button
          ...
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.showOnlineStatus ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white font-medium">
              {t('settings_page.privacy.show_activity', 'Show Activity Status')}
            </p>
            <p className="text-sm text-gray-400">
              {t('settings_page.privacy.show_activity_desc', 'Show your recent activity to others')}
            </p>
          </div>
          <button
            onClick={() =>
              setSettings((prev) => ({ ...prev, showActivityStatus: !prev.showActivityStatus }))
            }
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white font-medium">
              {t('settings_page.privacy.show_email', 'Show Email Address')}
            </p>
            <p className="text-sm text-gray-400">
              {t('settings_page.privacy.show_email_desc', 'Display email on your profile')}
            </p>
          </div>
          <button
          ...
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.showEmail ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
      </div>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        ...
        <div className="space-y-2">
          {[
            ...
            {
              value: 'followers',
              label: t('settings_page.privacy.dm_followers', 'Only people you follow'),
            },
            ...
            <label
              key={option.value}
              className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${
                settings.allowDirectMessages === option.value
                  ? 'bg-purple-500/20 border border-purple-500/50'
                  : 'bg-white/5 hover:bg-white/10'
              }`}
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...prev,
                    allowDirectMessages: e.target.value as 'everyone' | 'followers' | 'none',
                  }))
                }
                className="w-4 h-4 accent-purple-500"
              />
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        ...
        {settings.blockedUsers.length === 0 ? (
          <p className="text-gray-400 text-sm">
            {t('settings_page.privacy.no_blocked', 'No blocked users')}
          </p>
        ) : (
          <div className="space-y-2">
  ...
  const renderDisplaySettings = () => (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            >
              <option.icon
                className={`w-6 h-6 ${settings.theme === option.value ? 'text-purple-400' : 'text-gray-400'}`}
              />
              <span className={settings.theme === option.value ? 'text-white' : 'text-gray-400'}>
                {option.label}
              </span>
            </button>
          ))}
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <h3 className="text-white font-medium mb-4">
          {t('settings_page.display.posts_per_page', 'Posts Per Page')}
        </h3>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <h3 className="text-white font-medium mb-4">
          {t('settings_page.display.default_sort', 'Default Sort Order')}
        </h3>
        <div className="flex gap-2">
          {[
            ...
            <button
              ...
              onClick={() =>
                setSettings((prev) => ({
                  ...prev,
                  defaultSort: option.value as 'newest' | 'popular' | 'trending',
                }))
              }
              className={`px-4 py-2 rounded-lg transition-colors ${
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white font-medium">
              {t('settings_page.display.compact_mode', 'Compact Mode')}
            </p>
            <p className="text-sm text-gray-400">
              {t('settings_page.display.compact_mode_desc', 'Show more content with less spacing')}
            </p>
          </div>
          <button
          ...
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.compactMode ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        ...
        <textarea
          ...
          className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-purple-500 resize-none"
          placeholder={t(
            'settings_page.content.signature_placeholder',
            'Your signature will appear at the bottom of your posts...'
          )}
        />
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <h3 className="text-white font-medium mb-4">
          {t('settings_page.content.default_category', 'Default Post Category')}
        </h3>
        <select
        ...
        >
          <option value="general">
            {t('community_page.categories.general', 'General Discussion')}
          </option>
          ...
          <option value="governance">
            {t('community_page.categories.governance', 'Governance')}
          </option>
        </select>
      </div>
      ...
      <div className="p-4 bg-white/5 rounded-xl border border-white/10">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white font-medium">
              {t('settings_page.content.auto_save', 'Auto-save Drafts')}
            </p>
            <p className="text-sm text-gray-400">
              {t('settings_page.content.auto_save_desc', 'Automatically save your posts as drafts')}
            </p>
          </div>
          <button
            onClick={() =>
              setSettings((prev) => ({ ...prev, autoSaveDrafts: !prev.autoSaveDrafts }))
            }
            className={`w-12 h-6 rounded-full transition-colors relative ${settings.autoSaveDrafts ? 'bg-green-500' : 'bg-gray-600'}`}
          >
            <span
              className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${settings.autoSaveDrafts ? 'left-7' : 'left-1'}`}
            />
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">
              {t('settings_page.title', 'Settings')}
            </h1>
            <p className="text-gray-400 mt-1">
              {t('settings_page.subtitle', 'Manage your account preferences')}
            </p>
          </div>
          <motion.button
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">
                  {t('settings_page.delete_modal.title', 'Delete Account')}
                </h3>
                <p className="text-sm text-gray-400">
                  {t('settings_page.delete_modal.subtitle', 'This action cannot be undone')}
                </p>
              </div>
            </div>
            <p className="text-gray-300 mb-6">
              {t(
                'settings_page.delete_modal.warning',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          ...
          >
            <h3 className="text-xl font-bold text-white mb-4">
              {t('settings_page.block_modal.title', 'Block User')}
            </h3>
            <input
              type="text"
              value={blockUsername}
