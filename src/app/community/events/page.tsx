...
export default function EventsPage() {
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case 'online':
        return t('community_page.events.types.online');
      case 'offline':
        return t('community_page.events.types.offline');
      case 'hackathon':
        return t('community_page.events.types.hackathon');
      case 'workshop':
        return t('community_page.events.types.workshop');
      default:
        return t('community_page.events.types.event');
    }
  };
      ...
      <div className="relative z-10">
        <div className="container mx-auto px-4 py-8">
          ...
          >
            {[
              {
                label: t('community_page.events.stats.upcoming'),
                value: loading ? '-' : upcomingEvents.length,
                icon: Calendar,
                color: 'text-blue-400',
              },
              {
                label: t('community_page.events.stats.total_participants'),
                value: loading
                  ? '-'
              ...
              {
                label: t('community_page.events.stats.past_events'),
                value: loading ? '-' : pastEvents.length,
                icon: Star,
                color: 'text-yellow-400',
              },
              {
                label: t('community_page.events.stats.global_cities'),
                value: '20+',
                icon: Globe,
              ...
              >
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <span
                      className={`px-3 py-1 bg-gradient-to-r ${getTypeColor(event.type)} text-white text-sm rounded-full`}
                    >
                  ...
                  <div className="space-y-2 text-sm text-gray-400 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        ...
                        {t('community_page.events.people')}
                        {event.maxParticipants &&
                          ` / ${event.maxParticipants}${t('community_page.events.people')}`}
                        {event.status === 'past'
                          ? t('community_page.events.participated')
                          : t('community_page.events.registered')}
                      </span>
                    </div>
                    {event.prize && (
                      <div className="flex items-center gap-2 text-yellow-400">
                        ...
                        <span>
                          {t('community_page.events.prize_pool')}: {event.prize}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    ...
                    {event.status === 'past'
                      ? t('community_page.events.ended')
                      : registering === event.id
                        ? t('community_page.events.registering')
                        : t('community_page.events.register_now')}
                  </button>
                </div>
              </motion.div>
            ))}
