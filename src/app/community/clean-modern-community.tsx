...
export default function CleanModernCommunity() {
  ...
  const loadActiveMembers = async () => {
    try {
      ...
      setActiveMembers([
        ...
        {
          name: 'CryptoQueen',
          role: t('community_page.levels.community_leader'),
          reputation: 8920,
        },
  ...
  const loadTrendingPosts = async () => {
    try {
      const response = await barongAPI.get(
        `/public/community/trending-posts?limit=4&type=${activeTab}`
      );
      const data = response.data;
      if (data.success) {
  ...
  // 使用真实数据或后备数据
  const displayTrendingPosts =
    trendingPosts.length > 0
      ? trendingPosts
      : [
          {
            id: null,
            title: 'Quantum Mainnet Launch Announcement',
            replies: 234,
            views: 5420,
            isPinned: true,
          },
          ...
          {
            id: null,
            title: 'Governance Proposal #12: Fee Structure Update',
            replies: 156,
            views: 3200,
            isPinned: false,
          },
          {
            id: null,
            title: 'Weekly Development Update - Week 52',
            replies: 67,
            views: 1800,
            isPinned: false,
          },
  ...
  const activeUsers =
    activeMembers.length > 0
      ...
      : [
          {
            name: 'QuantumDev',
            role: t('community_page.levels.core_developer'),
            reputation: 9850,
            isOnline: true,
          },
          {
          ...
          {
            name: 'BlockchainBob',
            role: t('community_page.levels.senior_member'),
            reputation: 7650,
            isOnline: false,
          },
          {
            name: 'DeFiAlice',
            role: t('community_page.levels.defi_expert'),
            reputation: 6890,
            isOnline: true,
          },
        ];
    ...
    <div className="min-h-screen relative">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            {
              label: t('community_page.stats.members'),
              value:
              ...
              icon: Users,
              color: colors.accent.cyan,
              change: statsGrowth.members,
            },
            {
              label: t('community_page.stats.posts'),
              value:
              ...
              icon: MessageSquare,
              color: colors.secondary,
              change: statsGrowth.posts,
            },
            {
              label: t('community_page.stats.active_today'),
              value:
                stats.activeToday > 1000
                  ? `${(stats.activeToday / 1000).toFixed(1)}K`
                  : stats.activeToday.toString(),
              icon: Activity,
              color: colors.accent.green,
              change: statsGrowth.activeToday,
            },
            {
              label: t('community_page.stats.topics'),
              value:
                stats.totalTopics > 1000
                  ? `${(stats.totalTopics / 1000).toFixed(1)}K`
                  : stats.totalTopics.toString(),
              icon: Hash,
              color: colors.primary,
              change: statsGrowth.topics,
            },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3
                          className="font-semibold mb-1 transition-colors group-hover:text-purple-300"
                          style={{
                            color: colors.text.primary,
                            fontFamily: "'Space Grotesk', sans-serif",
                          }}
                ...
                {displayTrendingPosts.map((topic, i) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4
                          className="font-medium mb-2 line-clamp-2 transition-colors group-hover:text-purple-300"
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <a
                ...
                className="flex items-center justify-center gap-1 mt-4 py-2 text-sm transition-colors hover:opacity-80"
                style={{ color: colors.secondary }}
              >
            ...
            >
              ...
              <div className="space-y-2">
                {[
                  {
                    name: t('community_page.quick_links.guidelines'),
                    href: '/community/guidelines',
                  },
                  { name: t('community_page.quick_links.faq'), href: '/community/faq' },
                  {
                    name: t('community_page.quick_links.bug_bounty'),
            ...
            >
              ...
              <div className="space-y-3">
                {[
                  {
                    label: t('community_page.community_stats.total_posts'),
                    value: stats.totalPosts.toLocaleString(),
                  },
                  {
                    label: t('community_page.community_stats.total_members'),
                    value: stats.totalMembers.toLocaleString(),
                  },
                  {
                    label: t('community_page.community_stats.online_now'),
