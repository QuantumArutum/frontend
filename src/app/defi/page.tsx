...
export default function DeFiPage() {
  ...
  const [modalType, setModalType] = useState<'liquidity' | 'lending' | 'staking' | 'farm'>(
    'liquidity'
  );
  const [selectedPool, setSelectedPool] = useState<
    LiquidityPool | LendingPool | StakingPool | YieldFarm | null
  >(null);
  ...
  const formatLastUpdated = () => {
    ...
    if (diff < 3600)
      return `${t('defi_page.updated')} ${Math.floor(diff / 60)}${t('defi_page.minutes_ago')}`;
  ...
  const openModal = (
    ...
    pool: LiquidityPool | LendingPool | StakingPool | YieldFarm
  ) => {
    setModalType(type);
  ...
  const handleAction = () => {
    alert(
      `${t('defi_page.modal.action_success')}\nType: ${modalType}\nAmount: ${amount}\n\n${t('defi_page.modal.demo_mode')}`
    );
    setShowModal(false);
  };
  ...
  const tabs = [
    {
      id: 'liquidity',
      label: t('defi_page.tabs.liquidity'),
      ...
      <div className="relative z-10 pt-24 pb-12 px-4 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex items-center gap-6 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {lastUpdated && <span className="text-gray-500 text-xs">{formatLastUpdated()}</span>}
            </div>
          </div>
        </div>
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-center justify-between">
            ...
            <button
              onClick={handleRetry}
              className="text-sm text-red-400 hover:text-white transition-colors flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            {
              label: t('defi_page.stats.liquidity'),
              value: liquidityPools.length,
              icon: Droplets,
              color: 'text-blue-400',
            },
            {
              label: t('defi_page.stats.lending'),
              value: lendingPools.length,
              icon: Landmark,
              color: 'text-green-400',
            },
            {
              label: t('defi_page.stats.staking'),
              value: stakingPools.length,
              icon: Lock,
              color: 'text-purple-400',
            },
            {
              label: t('defi_page.stats.farms'),
              value: yieldFarms.length,
              icon: Sprout,
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">
                        {t('defi_page.liquidity.total_liquidity')}
                      </span>
                      <span className="text-white font-mono">
                        ${formatNumber(pool.total_liquidity || 0)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                  ...
                  <thead>
                    <tr className="border-b border-white/10 bg-white/5">
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.asset')}
                      </th>
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.total_supply')}
                      </th>
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.total_borrowed')}
                      </th>
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.supply_apy')}
                      </th>
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.borrow_apy')}
                      </th>
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.utilization')}
                      </th>
                      <th className="text-left py-4 px-6 text-gray-400 font-medium text-sm">
                        {t('defi_page.lending.action')}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {lendingPools.map((pool, index) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-white font-mono">
                          {formatNumber(pool.total_supply || 0)}
                        </td>
                        <td className="py-4 px-6 text-white font-mono">
                          {formatNumber(pool.total_borrowed || 0)}
                        </td>
                        <td className="py-4 px-6 text-green-400 font-medium">
                          {((pool.supply_rate || 0) * 100).toFixed(2)}%
                        </td>
                        <td className="py-4 px-6 text-red-400 font-medium">
                          {((pool.borrow_rate || 0) * 100).toFixed(2)}%
                        </td>
                        <td className="py-4 px-6 text-yellow-400 font-medium">
                          {((pool.utilization_rate || 0) * 100).toFixed(1)}%
                        </td>
                        <td className="py-4 px-6">
                ...
                >
                  ...
                  <div className="space-y-3 mb-6 bg-white/5 rounded-xl p-4">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-mono">
                        {pool.duration_days} {t('defi_page.staking.days')}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
              ...
              {yieldFarms.map((farm, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm font-mono">
                        {t('defi_page.farming.lp')}: {farm.lp_token}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold border ${farm.status === 'ACTIVE' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}
                      >
                        {farm.status}
                      </span>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center mb-6 bg-white/5 rounded-xl p-4">
                    <div>
                      <p className="text-gray-400 text-xs mb-1">
                        {t('defi_page.farming.total_staked')}
                      </p>
                      <p className="text-white font-mono font-medium">
                        {formatNumber(farm.total_staked || 0)}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-xs mb-1">
                        {t('defi_page.farming.reward_token')}
                      </p>
                      <p className="text-white font-mono font-medium">{farm.reward_token}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-xs mb-1">
                        {t('defi_page.farming.multiplier')}
                      </p>
                  ...
                  >
                    ...
                    {farm.status === 'ACTIVE'
                      ? t('defi_page.farming.stake_lp')
                      : t('defi_page.farming.farm_ended')}
                  </button>
                </motion.div>
              ))}
              {yieldFarms.length === 0 && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              <h3 className="text-xl font-bold text-white mb-4">
                {modalType === 'liquidity'
                  ? t('defi_page.modal.add_liquidity')
                  : modalType === 'lending'
                    ? t('defi_page.modal.lending_action')
                    : modalType === 'staking'
                      ? t('defi_page.modal.stake_tokens')
                      : t('defi_page.modal.stake_lp')}
              </h3>
                ...
                <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                  {modalType === 'liquidity' && (
                    <p className="text-cyan-400 font-medium text-lg">
                      {(selectedPool as LiquidityPool).token_a}/
                      {(selectedPool as LiquidityPool).token_b}
                    </p>
                  )}
                  {modalType === 'lending' && (
                    <p className="text-green-400 font-medium text-lg">
                      {(selectedPool as LendingPool).asset}
                    </p>
                  )}
                  {modalType === 'staking' && (
                    <p className="text-purple-400 font-medium text-lg">
                      {(selectedPool as StakingPool).token}
                    </p>
                  )}
                  {modalType === 'farm' && (
                    <p className="text-orange-400 font-medium text-lg">
                      {(selectedPool as YieldFarm).name}
                    </p>
                  )}
                  ...
                  <>
                    <div>
                      <label className="block text-gray-400 text-sm mb-2">
                        {(selectedPool as LiquidityPool).token_a} {t('defi_page.modal.amount')}
                      </label>
                      <input
                        type="number"
                        value={amount}
                    ...
                    <div>
                      <label className="block text-gray-400 text-sm mb-2">
                        {(selectedPool as LiquidityPool).token_b} {t('defi_page.modal.amount')}
                      </label>
                      <input
                        type="number"
                        value={amountB}
