...
const MovieTicketPage = () => {
  ...
  const calculateTotal = () => {
    ...
    return selectedSeats.reduce(
      (total, seat) => total + selectedShowtime.ticket_price * seat.price_modifier,
      0
    );
  };
        ...
        <div className="flex justify-center mb-8">
          <div className="flex items-center space-x-4">
            {[
              ...
              const isCompleted =
                ['movies', 'showtimes', 'seats', 'payment'].indexOf(currentStep) > index;
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-gray-600 text-sm mb-2">
                      {t('movies_page.director')}: {movie.director}
                    </p>
                    <p className="text-gray-600 text-sm mb-2">
                      {t('movies_page.genre')}: {movie.genre}
                    </p>
                    <p className="text-gray-600 text-sm mb-2">
                      {t('movies_page.duration')}: {movie.duration_minutes}{' '}
                      {t('movies_page.minutes')}
                    </p>
                    <div className="flex items-center mb-3">
                      <Star className="h-4 w-4 text-yellow-400 mr-1" />
                      <span className="text-sm font-medium">{movie.imdb_rating}</span>
                      <span className="text-gray-500 text-sm ml-2">
                        {t('movies_page.rotten_tomatoes')} {movie.rotten_tomatoes_score}%
                      </span>
                    </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold">
                {t('movies_page.select_showtime_for', { title: selectedMovie?.title })}
              </h2>
              <button
                onClick={() => setCurrentStep('movies')}
                className="text-blue-600 hover:text-blue-800"
              >
                {t('movies_page.back_to_movies')}
              </button>
            </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-gray-50 p-3 rounded">
                      <div className="flex items-center">
                        ...
                        <span className="font-medium">{showtime.show_time}</span>
                      </div>
                    ...
                    <div className="bg-gray-50 p-3 rounded">
                      <span className="font-medium">
                        {showtime.screen?.screen_type || t('movies_page.standard')}
                      </span>
                  ...
                  <div className="mt-4 flex justify-end">
                    ...
                    >
                      {showtime.available_seats === 0
                        ? t('movies_page.sold_out')
                        : t('movies_page.select_seats_btn')}
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-blue-600 hover:text-blue-800"
              >
                {t('movies_page.back_to_showtimes')}
              </button>
            </div>
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="mb-6">
                ...
                <p className="text-gray-600">
                  {selectedShowtime?.cinema?.cinema_name} - {selectedShowtime?.show_date}{' '}
                  {selectedShowtime?.show_time}
                </p>
              </div>
              <div className="text-center mb-8">
                <div className="bg-gray-800 text-white py-2 px-8 rounded-lg inline-block">
                  {t('movies_page.screen')}
                </div>
              </div>
              <div className="text-center py-8 text-gray-500">
                {t('movies_page.seat_selection_dev')}
              </div>
              {selectedSeats.length > 0 && (
                <div className="mt-6 bg-blue-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center">
                    <span className="font-medium">
                      {t('movies_page.total')}: {calculateTotal().toFixed(2)} QAU
                    </span>
                    <button
                      onClick={() => setCurrentStep('payment')}
                      className="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700"
                    >
                      {t('movies_page.confirm_seats')}
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span>{t('movies_page.movie')}:</span>
                  <span className="font-medium">{selectedMovie?.title}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t('movies_page.cinema')}:</span>
                  <span className="font-medium">{selectedShowtime?.cinema?.cinema_name}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t('movies_page.time')}:</span>
                  <span className="font-medium">
                ...
                <div className="flex justify-between text-lg font-bold">
                  <span>{t('movies_page.total')}:</span>
                  <span className="text-blue-600">{calculateTotal().toFixed(2)} QAU</span>
                </div>
              </div>
              <button className="w-full mt-6 bg-blue-600 text-white py-3 rounded-md hover:bg-blue-700 font-medium">
                {t('movies_page.confirm_pay')} {calculateTotal().toFixed(2)} QAU
              </button>
            </div>
          </div>
        )}
