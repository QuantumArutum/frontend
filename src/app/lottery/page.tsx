...
const QuantumLotteryPage = () => {
    ...
    <Card className="p-4">
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-sm">
        <div>
          ...
          <div className="text-xs text-gray-500">{t('lottery_page.sales')}</div>
        </div>
        <div>
          <div className="font-bold text-white">{result.total_bets.toLocaleString()}</div>
          <div className="text-xs text-gray-500">{t('lottery_page.total_bets')}</div>
        </div>
        <div>
          <div className="font-bold text-white">{formatCurrency(result.prize_pool)}</div>
          <div className="text-xs text-gray-500">{t('lottery_page.prize_pool')}</div>
        </div>
      </div>
    </Card>
  );
  ...
  const tabs = [
    { id: 'buy', label: t('lottery_page.tabs.buy'), icon: <Dice6 className="w-4 h-4" /> },
    { id: 'results', label: t('lottery_page.tabs.results'), icon: <Trophy className="w-4 h-4" /> },
    { id: 'mybets', label: t('lottery_page.tabs.mybets'), icon: <Target className="w-4 h-4" /> },
    { id: 'stats', label: t('lottery_page.tabs.stats'), icon: <TrendingUp className="w-4 h-4" /> },
  ];
    ...
    >
      ...
      {activeTab === 'buy' && (
        <div className="space-y-6">
          <Card>
            <CardHeader
              title={t('lottery_page.select_numbers')}
              icon={Dice6}
            ...
            <CardContent>
              ...
              <div className="bg-gradient-to-r from-purple-900/30 to-blue-900/30 rounded-xl p-4 border border-purple-500/20">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-white">{t('lottery_page.bet_info')}</h3>
                  <Badge variant="info" className="flex items-center gap-1">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm">{t('lottery_page.bets')}</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-purple-400">2.0 QAU</div>
                    <div className="text-gray-400 text-sm">{t('lottery_page.single_bet')}</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-green-400">2.0 QAU</div>
                    <div className="text-gray-400 text-sm">{t('lottery_page.total_amount')}</div>
                  </div>
                </div>
      ...
      {activeTab === 'mybets' && (
        <Card className="text-center py-16">
          <CardContent>
            ...
            <h3 className="text-xl font-semibold text-white mb-2">{t('lottery_page.no_bets')}</h3>
            <p className="text-gray-400 mb-6">{t('lottery_page.connect_wallet_view')}</p>
            <Button variant="primary" onClick={() => setActiveTab('buy')}>
              {t('lottery_page.bet_now')}
            </Button>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="mt-8 bg-gradient-to-r from-blue-900/30 to-purple-900/30 border-blue-500/20">
        <CardContent className="text-center py-8">
          ...
          <p className="text-gray-400 mb-6 max-w-2xl mx-auto">
            {t('lottery_page.quantum_security.desc')}
          </p>
