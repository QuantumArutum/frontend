 ...
 */
function getAllTranslationValues(obj: unknown, prefix = ''): Array<{ key: string; value: string }> {
...
function findKeysWithTerm(term: TechnicalTerm): string[] {
  ...
  return allValues.filter(({ value }) => containsTechnicalTerm(value, term)).map(({ key }) => key);
}
